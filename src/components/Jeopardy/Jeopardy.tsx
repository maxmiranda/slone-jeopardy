...
export class Jeopardy extends React.Component<{
  ...
  async componentDidUpdate(prevProps: any, prevState: any) {
    ...
    // Set a default target for score adjustments when players load in.
    if (
      !this.state.adjustScoreTarget &&
      this.props.participants.find((p) => !p.role || p.role === 'player')
    ) {
      const firstPlayer =
        this.props.participants.find((p) => !p.role || p.role === 'player')?.id;
      if (firstPlayer && firstPlayer !== this.state.adjustScoreTarget) {
        this.setState({ adjustScoreTarget: firstPlayer });
      }
    }
  }
  ...
  newGame = async (
    ...
    const combined: GameOptions = {
      ...
      ...this.state.settings,
      makeMeHost: this.props.mode === 'admin' ? true : this.state.settings.makeMeHost,
    };
  ...
  startDefaultGame = async () => {
    // Let the server load the bundled default CSV (no episode/filter/custom passed).
    this.newGame({});
  };
  ...
  adjustScore = (delta: number) => {
    if (!this.state.adjustScoreTarget) {
      return;
    }
    this.socket?.emit('JPD:adjustScore', {
      id: this.state.adjustScoreTarget,
      delta,
    });
  };
  ...
  submitAnswer = () => {
    if (!this.state.localAnswerSubmitted) {
      this.socket?.emit('JPD:answer', this.state.game?.currentQ, null);
  ...
  render() {
    const game = this.state.game;
    const categories = this.getCategories();
    const mode = this.props.mode ?? 'player';
    ...
    const selectedAdjustTarget =
      this.state.adjustScoreTarget || playerOptions[0]?.value || '';
    const players = participants.filter((p) => !p.role || p.role === 'player');
    const buzzOrder = game
      ? Object.entries(game.buzzes || {}).sort((a, b) => a[1] - b[1])
      : [];
    const canJudge =
      !game?.host || this.socket?.id === game?.host || isAdmin;
    return (
      <>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <Button
                size="xs"
                variant="light"
                onClick={() => this.socket?.emit('JPD:claimHost')}
                disabled={game?.host === this.socket?.id}
              >
                Claim host
              </Button>
              <Button
                ...
                variant="light"
                color="orange"
                onClick={() => this.socket?.emit('JPD:revealNow')}
                disabled={!game?.currentQ || Boolean(game?.currentAnswer)}
              >
                Start judging now
              </Button>
              <Button
              ...
              <Button
                ...
                variant="light"
                color="violet"
                onClick={() => this.setState({ showCustomModal: true })}
            ...
            <div style={{ marginTop: '8px' }}>
              <Title order={5}>Buzz order</Title>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {buzzOrder.length === 0 && <div>Waiting for buzzers…</div>}
                {buzzOrder.map(([id], idx) => {
                  const name =
            ...
            {isAdmin && (
              <div style={{ marginTop: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                <Title order={5} style={{ marginRight: '8px' }}>
                  Adjust scores
                </Title>
                <Select
                  placeholder="Player"
                  data={playerOptions}
                  value={selectedAdjustTarget}
                  onChange={(value) =>
                    this.setState({ adjustScoreTarget: value || '' })
                  }
                  searchable
                ...
                />
                <NumberInput
                  value={this.state.adjustScoreDelta}
                  onChange={(val) =>
                    this.setState({ adjustScoreDelta: Number(val) || 0 })
                  }
                  hideControls
                ...
                <Button
                  size="xs"
                  color="green"
                  onClick={() => this.adjustScore(Math.abs(this.state.adjustScoreDelta))}
                >
                  Add
                </Button>
                <Button
                  ...
                  variant="light"
                  onClick={() =>
                    this.adjustScore(-Math.abs(this.state.adjustScoreDelta))
                  }
                >
                  Subtract
                </Button>
        ...
        {!game && (
          ...
          >
            {isAdmin
              ? 'No game loaded. Use “New Game” above to start an episode.'
              : 'Waiting for a game to be started.'}
          </div>
        )}
        <div
          style={{
                    ...
                    {categories.map((cat, j) => {
                      ...
                      return (
                        <div
                          ...
                          id={id}
                          onClick={
                            clue && isAdmin ? () => this.pickQ(id) : undefined
                          }
                  ...
                  >
                    ...
                    <div className="" style={{ height: '60px' }}>
                      ...
                      game.questionEndTS ? (
                        <div style={{ paddingTop: '12px' }}>
                          You buzzed in! The host will award the question.
                        </div>
                      ) : null}
                    ...
                    {Boolean(game.questionEndTS) && (
                      <TimerBar
                        ...
                        secondary
                        submitAnswer={mode === 'player' ? this.submitAnswer : undefined}
        ...
        <Button
          onClick={() => {
            ...
            const arr = answers
              ...
              .filter(Boolean) as { id: string; correct: boolean | null }[];
            bulkJudge(arr);
            // Close the modal
            onClose();
          }}
