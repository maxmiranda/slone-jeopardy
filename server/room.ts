...
// Load a default question set from the bundled CSV so new games have content without user uploads.
function loadDefaultCsvGame() {
  try {
    const csvPath = path.resolve('./public/final_game_export.csv');
    if (!fs.existsSync(csvPath)) {
      return null;
    }
    const csv = fs.readFileSync(csvPath, 'utf-8');
    const parsed = Papa.parse<any>(csv, { header: true });
    let round = '';
    let cat = '';
    let curX = 0;
    let curY = 0;
    ...
    console.warn('[DEFAULT CSV]', err);
    return null;
  }
}

const defaultCsvGame = loadDefaultCsvGame();
...
export class Room {
  ...
  private roles: Record<string, 'player' | 'admin' | 'display' | 'spectator'> = {};
  ...
  constructor(
    ...
    io.of(roomId).on('connection', (socket: Socket) => {
      const role =
        (socket.handshake.query?.role as
          | 'player'
          | 'admin'
          | 'display'
          | 'spectator'
      ...
      if (
        isPlayer &&
        !this.getAllPlayers().find((p) => p.id === socket.id)
      ) {
      ...
      socket.on('JPD:start', (options, data) => {
        if (role !== 'admin' && socket.id !== this.settings.host) {
          return;
        }
      ...
      socket.on('JPD:buzz', () => {
        ...
        this.jpd.answers[socket.id] = '';
        this.sendState();
      });
      socket.on('JPD:answer', (question, answer) => {
        // Manual answer entry has been disabled; treat this as a pass.
        if (!isPlayer) {
          return;
        }
      ...
      socket.on('JPD:bulkJudge', (data) => {
        ...
        // Process in buzz order so scores are consistent, and stop after first correct (auto-skip rest).
        const buzzOrder = Object.entries(this.jpd.public.buzzes || {})
          .sort((a, b) => a[1] - b[1])
          .map(([id]) => id)
          .filter((id) => !(id in this.jpd.public.judges));

        let foundCorrect = false;
        for (const id of buzzOrder) {
          const decision = data.find((d: any) => d.id === id);
          if (!decision || decision.correct === undefined) {
            continue;
          }
          const decisionBool =
            decision.correct === null ? null : Boolean(decision.correct);
          this.judgeAnswer(
            socket,
            { currentQ: this.jpd.public.currentQ, id, correct: decisionBool },
            true /* suppressAdvance */,
          );
        ...
        // Auto-skip the remaining players after the first correct.
        if (foundCorrect) {
          buzzOrder.forEach((id) => {
            if (!(id in this.jpd.public.judges)) {
              this.jpd.public.judges[id] = null;
            }
          });
        } else {
          // If no correct was chosen but all judged, allow advancing.
          buzzOrder.forEach((id) => {
            if (!(id in this.jpd.public.judges)) {
              this.jpd.public.judges[id] = null;
            }
        ...
        // Advance to next question after bulk judging.
        this.jpd.public.currentJudgeAnswerIndex = Object.keys(
      ...
      socket.on('JPD:adjustScore', (data) => {
        if (!data || typeof data.delta !== 'number' || !data.id) {
          return;
        }
        // Only host/admin may adjust scores
        if (this.settings.host && socket.id !== this.settings.host && role !== 'admin') {
          return;
        }
        if (!this.jpd.public.scores[data.id]) {
          this.jpd.public.scores[data.id] = 0;
        }
        this.jpd.public.scores[data.id] += data.delta;
        this.sendRoster();
        this.sendState();
      });
      socket.on('JPD:undo', () => {
      ...
      socket.on('JPD:claimHost', () => {
        if (role === 'admin') {
          this.settings.host = socket.id;
          this.sendState();
        }
      });
      socket.on('JPD:revealNow', () => {
        if (this.settings.host && socket.id !== this.settings.host) {
          return;
        }
        if (!this.jpd.public.canNextQ && this.jpd.public.currentQ) {
          this.revealAnswer();
        }
      });
      socket.on('CMD:chat', (data: string) => {
        ...
        const chatMsg = {
          id: socket.id,
          name: sender?.name || this.roles[socket.id],
          msg: data,
        };
        this.addChatMessage(socket, chatMsg);
      });
      socket.on('disconnect', () => {
        delete this.roles[socket.id];
  ...
  getConnectedPlayers = () => {
    ...
    return this.roster.filter(
      (p) => p.connected && (!p.role || p.role === 'player'),
    );
  };
  ...
  getActivePlayers = () => {
    ...
    return this.roster.filter((p) => !p.role || p.role === 'player');
  };
  ...
  handleReconnect = (newId: string, oldId: string) => {
    ...
    if (this.roles[oldId]) {
      this.roles[newId] = this.roles[oldId];
      delete this.roles[oldId];
    }
  ...
  loadEpisode = (socket: Socket, options: GameOptions, custom: string) => {
    ...
    if (custom) {
      ...
      // Default: use bundled CSV game if available and no explicit episode requested.
      if (!number && !filter && defaultCsvGame) {
        loadedData = JSON.parse(JSON.stringify(defaultCsvGame));
      } else {
  ...
  revealAnswer = () => {
    ...
    // Add placeholder answers that show buzz order for judges
    Object.keys(this.jpd.public.buzzes).forEach((key, index) => {
      this.jpd.answers[key] = `Buzz #${index + 1}`;
    });
  ...
  judgeAnswer = (
    ...
    if (
      this.settings.host &&
      socket &&
      socket?.id !== this.settings.host &&
      this.roles[socket.id] !== 'admin'
    ) {
      // Not the host or admin
      return;
    }
  ...
  submitWager = (id: string, wager: number) => {
    if (!this.getActivePlayers().find((p) => p.id === id)) {
      return;
    }
