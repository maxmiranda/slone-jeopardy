...
app.use('*', cors());
app.use('*', compress());
// Static assets from the build output; let other routes fall through to SPA handler.
app.use('/assets/*', serveStatic({ root: './build' }));
app.use('/jeopardy*', serveStatic({ root: './build' }));
app.use('/example.csv', serveStatic({ root: './build' }));
app.use('/robots.txt', serveStatic({ root: './build' }));
...
// SPA fallback for client routes (admin, display, etc.)
